...
import c3_hotel_booking.factory.RoomFactory;
import c3_hotel_booking.model.Guest;
import c3_hotel_booking.observer.EmailNotifier;
import c3_hotel_booking.observer.SmsNotifier;
...
public class HotelManagementDemo {
	public static void main(String[] args) {
		...
		BookingManager hotelManager = new BookingManager(roomService, bookingService, payment);
		...
		hotelManager.bookRoom(guest1, RoomType.DOUBLE, RoomStyle.DELUXE, LocalDate.now(), LocalDate.now().plusDays(3));

		System.out.println("\n----------- SCENARIO 2: Attempt to book the same room (State Pattern) -----------");
		Guest guest2 = new Guest("G456", "Jane Smith", "jane.smith@example.com");
		hotelManager.bookRoom(guest2, RoomType.DOUBLE, RoomStyle.DELUXE, LocalDate.now(), LocalDate.now().plusDays(2));
		...
		hotelManager.bookRoom(guest3, RoomType.SINGLE, RoomStyle.STANDARD, LocalDate.now(), LocalDate.now().plusDays(1));
	}
}
