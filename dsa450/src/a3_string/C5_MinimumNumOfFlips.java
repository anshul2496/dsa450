...
public class C5_MinimumNumOfFlips {
	public static void main(String[] args) {
		String s = "001";
		int flips = 0;
		// idea is to calculate string flips when the string starts from 1 and then use (length - calculated) to get the 
		// flips when the string starts from 0 finally return the minimum
		for (int i = 0; i < s.length(); i++) {
			if (i % 2 == 0) {
				// If there is a 0 at even index then we need to flip it
				if (s.charAt(i) == '0')  // Can be 1 as well, doesn't matter
					flips++;
			} else {
				// If there is a 1 at odd index then we need to flip it
				if (s.charAt(i) == '1') // Can be 0 as well, doesn't matter
					flips++;
			}
		}
